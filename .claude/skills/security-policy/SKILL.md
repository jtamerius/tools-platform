...
| Lambda URL (CFN) | `apps/weather_app/ensemble-plumes/infrastructure/finance-tracker/template.yaml` — `Cors.AllowOrigins` |
...
grep -rn "allowOrigins\|AllowOrigins" infra/cdk/lib/stacks/ apps/weather_app/
```
...
| Finance tracker Lambda role | `apps/weather_app/ensemble-plumes/infrastructure/finance-tracker/template.yaml` — `LambdaRole` |
...
- [ ] Finance tracker handler still calls `cognito.get_user(AccessToken=token)` and returns 401 on failure — check `apps/weather_app/finance-tracker/lambda_handler.py`.
...
grep -B5 "allowOrigins.*\*\|AllowOrigins.*\*" \
  ...
  apps/weather_app/ensemble-plumes/infrastructure/finance-tracker/template.yaml
