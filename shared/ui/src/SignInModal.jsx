...
/**
 * Shared sign-in modal used by all platform apps.
 *
 * Handles four views: signin, newpassword, forgot, reset.
 *
 * Props:
 *   isOpen       {boolean}  — Controls visibility
 *   onClose      {function} — Called when the modal should close
 *   onSignIn     {function} — async (email, password) => void
 *   onNewPassword {function} — async (cognitoUser, newPassword) => void
 */
export function SignInModal({ isOpen, onClose, onSignIn, onNewPassword }) {
          ...
          <>
            ...
            <form onSubmit={handleNewPasswordSubmit} noValidate>
              ...
              <button style={{ ...s.submitBtn, background: isSubmitting ? '#555' : 'var(--accent)' }}
                ...
                onMouseEnter={e => { if (!isSubmitting) e.currentTarget.style.background = 'var(--accent-hover)' }}
                onMouseLeave={e => { if (!isSubmitting) e.currentTarget.style.background = 'var(--accent)' }}
              >{isSubmitting ? 'Setting password…' : 'Set password & sign in'}</button>
            </form>
          </>
        )}
          ...
          <>
            ...
            <form onSubmit={handleForgotSubmit} noValidate>
              ...
              <button style={{ ...s.submitBtn, background: isSubmitting ? '#555' : 'var(--accent)' }}
                ...
                onMouseEnter={e => { if (!isSubmitting) e.currentTarget.style.background = 'var(--accent-hover)' }}
                onMouseLeave={e => { if (!isSubmitting) e.currentTarget.style.background = 'var(--accent)' }}
              >{isSubmitting ? 'Sending…' : 'Send reset code'}</button>
            </form>
          ...
          <>
            ...
            <form onSubmit={handleResetSubmit} noValidate>
              ...
              <button style={{ ...s.submitBtn, background: isSubmitting ? '#555' : 'var(--accent)' }}
                ...
                onMouseEnter={e => { if (!isSubmitting) e.currentTarget.style.background = 'var(--accent-hover)' }}
                onMouseLeave={e => { if (!isSubmitting) e.currentTarget.style.background = 'var(--accent)' }}
              >{isSubmitting ? 'Updating…' : 'Set new password'}</button>
            </form>
            <button style={s.textLink} type="button" onClick={() => { setError(null); setInfo(null); setView('forgot') }}>
              ← Resend code
            </button>
...
const s = {
  backdrop: { position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' },
  dialog: { background: 'var(--surface)', borderRadius: 'var(--radius-lg)', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', width: '100%', maxWidth: '400px', padding: '32px 28px 28px', position: 'relative' },
  closeBtn: { position: 'absolute', top: '14px', right: '14px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-faint)', fontSize: '1rem', padding: '4px 6px', borderRadius: '4px', lineHeight: 1 },
  title: { fontSize: '1.2rem', fontWeight: 700, letterSpacing: '-0.02em', color: 'var(--text)', marginBottom: '6px' },
  subtitle: { fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '24px' },
  fields: { display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '20px' },
  label: { display: 'flex', flexDirection: 'column', gap: '5px', fontSize: '0.875rem', fontWeight: 500, color: 'var(--text)' },
  input: { padding: '9px 12px', borderRadius: 'var(--radius)', border: '1px solid var(--border)', fontSize: '0.9rem', color: 'var(--text)', background: 'var(--bg)', outline: 'none', transition: 'border-color 0.12s ease' },
  errorBox: { background: '#fff0f0', border: '1px solid #fca5a5', borderRadius: 'var(--radius)', padding: '10px 12px', fontSize: '0.85rem', color: '#b91c1c', marginBottom: '16px', lineHeight: 1.4 },
  infoBox: { background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 'var(--radius)', padding: '10px 12px', fontSize: '0.85rem', color: '#0369a1', marginBottom: '16px', lineHeight: 1.4 },
  submitBtn: { width: '100%', padding: '10px', borderRadius: 'var(--radius)', border: 'none', color: '#fff', fontSize: '0.9rem', fontWeight: 600, transition: 'background 0.12s ease', marginBottom: '16px' },
  textLink: { display: 'block', width: '100%', textAlign: 'center', background: 'none', border: 'none', padding: '4px', fontSize: '0.8rem', color: 'var(--text-muted)', cursor: 'pointer' },
}
