...
export default function CamMap() {
  ...
  useEffect(() => {
    const map = L.map(containerRef.current, { zoomControl: true }).setView([37.42, -107.83], 10)
    ...
    ALL_CAMS.forEach(cam => {
      ...
      const typeLabel = cam.type === 'rwis' ? 'Weather station' : `Traffic cam · ${cam.driveMin} min to resort`
      marker.bindPopup(
        `<b style="color:#000">${cam.id}</b><br>` +
        `MP ${cam.mp} · US-550<br>` +
        `<span style="color:#555">${typeLabel}</span>`,
        { className: 'purg-popup' }
      )
