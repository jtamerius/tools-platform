...
export function useHailData(metro, startDate, endDate) {
  ...
  const { cells, stats } = useMemo(() => {
    ...
    const stats = {
      ...
      solarBySize: {
        moderate:    cells.filter(c => c.maxMeshMm >= 25 && c.maxMeshMm < 35).reduce((s, c) => s + c.totalSolarExposed, 0),
        significant: cells.filter(c => c.maxMeshMm >= 35 && c.maxMeshMm < 50).reduce((s, c) => s + c.totalSolarExposed, 0),
        severe:      cells.filter(c => c.maxMeshMm >= 50).reduce((s, c) => s + c.totalSolarExposed, 0),
      },
    };
